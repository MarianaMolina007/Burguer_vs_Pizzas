...
def make_tfrec(dataset_dir,tfrec_dir,img_shard):
  ...
  zip_ref = zipfile.ZipFile('burger-vs-pizza.zip', 'r')
  ...
  for img in imageList:
    class_name,_ = img.split('_',1)
    if class_name == 'pizzaa':
        pizzaImages.append(img)
    else:
        burgerImages.append(img)
  print(len(burgerImages))
  print(len(pizzaImages))
  assert(len(pizzaImages)==len(burgerImages)), 'Number of images in each class do not match'

  # define train/test split as 80:20
  split = int(len(burgerImages) * 0.2)

  testImages = burgerImages[:split] + pizzaImages[:split]
  trainImages = burgerImages[split:] + pizzaImages[split:]
